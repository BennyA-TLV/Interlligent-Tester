...
from TestingEquipment.NetworkAnaLyzer.NetworkAnalyzer import *
...
# N50xxB device class
    ...
    # Main N50xxB function

class E50XXB(NetworkAnalyzer):
...
def E506x_System_Test(target_ip, worker=None, logger=None, run_win_update=True, run_firmware_update=True):
    ...
    time.sleep(5)
    try:
        device = E50XXB(target_ip, logger)
        if device.connect():
            report_step("E50xxB Device is replaying", "PASS", 8, results_list, " ", None, worker, logger)
            device.disconnect()
        else:
        ...
        if  device.connect() and windows_control.connect():
            ...
            if url != "No url":
                ...
                if run_firmware_update:
                    ...
                    if (firmwareWebVer.get('revision') != firmwareUnitVer) and (
                            requested_os.lower() in firmwareWebVer.get('os').lower()):
                        report_step("User cancel firmware update Need to Update Firmware manually", "INFO", 55, results_list,
                                    f"Need to Update Firmware manually!! - Device {firmwareUnitVer}", None, worker, logger)
...
def E50xx_System_Test(target_ip, worker=None, logger=None):
    ...
    if download_path is None:
        download_path = Path.home() / "Downloads"
    download_default = Path.home() / "Downloads"
...
def is_e506x(model):
    return bool(re.fullmatch(r"E506\d\w", model))

def get_e50_family(model):

    model = model.upper().strip()
    if re.match(r"^E50\d{2}B$", model):
        return "E50xxB"
    return None

def network_response(target_ip, worker=None, logger=None, timeout=600):

    network = E50XXB(target_ip, logger)
    return device_information(network,  worker=worker, logger=logger, timeout=timeout)
